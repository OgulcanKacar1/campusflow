...
- [ ] Öğrenciler `sprint_mode = 'team'` olduğunda kendi takımlarının kanbanını yönetebiliyor, diğer durumlarda yalnızca okuyabiliyor. *(UI var, yetki modları yapılandırılacak.)*
...
- [ ] Hangi noktalarda optimistic update kabul edilebilir belirle; API hata alırsa geri alma akışı ekle. *(Şimdilik tam refresh.)*
